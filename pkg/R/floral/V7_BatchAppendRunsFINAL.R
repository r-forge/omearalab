...
while(1<2) { #keep looping
  for (may.dir.index in sequence(length(MayDirs))) {
    ...
    files<-system("ls *11043/*RSave", intern=TRUE)
    if(length(files)>0) {
      ...
      for (i in sequence(length(files))) {
        ...
        setwd(dir)
       # system("cp /Users/bomeara/Documents/MyDocuments/Active/OMearaLabR/pkg/R/floral/V7*R .")
        file.root<-paste(strsplit(gsub("_app\\d+total\\d+","", file), "\\.")[[1]][1:3],collapse=".")
        print(files.total[i])
...
appendParallelSSA(x0=x0, q.means=q.means, lambda.means=lambda.means, mu.means=mu.means, prev.history=history, t.additional=', t.additional, ', maxWallTime=Inf, verbose=F, file.string="', file.root, '", print.freq=100000, rescale.species=rescale.species, yule.scale=0, t.rescale=t.rescale, x0.rescale=x0.rescale, ntax.old.scale=11043)
