...
doRunForGeneratingModel <- function(rep, model.index, generating.models, testing.model.subset) {
  ...
  phy<-NULL
  while(is.null(phy)) {
    try(phy<-tree.musse(pars=pars, max.t=136, x0=1)) #set to same root age as our observed tree
  }
...
save(list=ls(), "AICWorkspace.RSave", compress=TRUE) #has all the local objects, incl fns
