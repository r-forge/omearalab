...
OMearaSSASave<-function(x0, q.vector, lambda.vector, mu.vector, tf, maxWallTime, verbose=TRUE, print.freq=100, full.history=TRUE, rescale.species=NULL, yule.scale=0, history.steps.to.save=seq(from=1,to=floor(tf),length.out=floor(tf)), t.rescale=tf, x0.rescale=NULL, ntax.old.scale=NA) { #turn off full.history to save on memory
  ...
  if(!is.null(rescale.species)) {
    ...
    if(!is.na(ntax.old.scale)) {
      ...
      if(verbose) {
        print(paste("rescaling the scale factor itself by ", r.wanted/r.actual))
      }
  ...
  if(verbose) {
    ...
    print(all.matrix)
    save(list=ls(), file="all.matrix.RDATA")
  }
  return(all.matrix)
}
