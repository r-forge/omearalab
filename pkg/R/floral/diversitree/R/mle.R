...
guess.constrained.start <- function(func, x.init, warn=TRUE) {
  ...
  if ( length(x.init) == n.cons ) {
    ...
    x.init <- x.init[arg.idx]
  	...
  	print(paste("length(x.init)=", length(x.init), "n.orig=",n.orig,"n.cons=",n.cons,"arg.idx=",arg.idx)) #added by BCO Brian O'Meara
	badposVector<-which(is.na(arg.idx))
	if (length(badposVector)>0) {
		for (badpositionindex in 1:length(badposVector)) {
			print(paste("no match to ",	names.cons[badposVector[badpositionindex] ], " in names.orig"))
		}
		print("names.orig:")
		print(names.orig)
		print("x.init:")
