...
constrain.parse <- function(formula, names.lhs, names.rhs, extra) {
  formula <- as.formula(formula)
  if ( length(formula) != 3L )
    stop("Invalid formula")
  lhs <- formula[[2]]
  rhs <- formula[[3]]
#print(paste("extra =",extra)) #added by BCO
  ...
  if ( !is.name(lhs) || is.na(match(as.character(lhs), names.lhs)) ) {
  	if (!is.name(lhs)) {
  		print(paste("LHS ",as.character(lhs)," !is.name"))
  	}
  	if (is.na(match(as.character(lhs), names.lhs))) {
  		print(paste("is.na(match(as.character(lhs), names.lhs)), as.character(lhs)=",as.character(lhs)," names.lhs=",names.lhs))
  	}
  ...
  if ( is.language(rhs) ) {
    ...
    if ( !ok ) { #SECTION CHANGED BY BRIAN O'MEARA BCO
      print(paste("names.rhs = ",names.rhs))		
	print(paste("names.lhs = ",names.lhs))
	print(paste("extra = ",extra))
	dput(extra)
...
#  print(paste("extra in for(formula..) =",extra)) #added by BCO

    res <- constrain.parse(formula, names.lhs, names.rhs, extra)
#  print(paste("extra after res <- constrain.parse =",extra)) #added by BCO
