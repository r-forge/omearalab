...
constraint.vector <- c("transonlyRedo")
...
for (i in sequence(length(net.div.vector))) {
    ...
    for (k in sequence(length(best.vector)))
		MakeRunFiles(constraint=constraint.vector[j], net.div=net.div.vector[i], tf=20, submit=TRUE, nrep=50, best.only=best.vector[k], x0.rescale=x0.rescale, x0=x0)
	}
}
