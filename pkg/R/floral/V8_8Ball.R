...
try(for (comboDecimal in 1:2^S) {
  ...
  combo.names <- append(combo.names, comboName)
...
for(i in sequence(2^S)) {
 if (comboAsBinaryVector(i, S)[1]==0 && comboAsBinaryVector(i, S)[2]==1) {
...
plot(g, layout=layout.kamada.kawai(g, start=start), vertex.color=colors.focal, vertex.label=combo.names, edge.arrow.size=0, edge.color=colors.edges, vertex.size=40)

plot(g, layout=layout.fruchterman.reingold(g, start=start), vertex.color=colors.focal, vertex.label=combo.names, edge.arrow.size=0, edge.color=colors.edges, vertex.size=40)
