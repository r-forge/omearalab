		...
		if(doRsync) {
			for (rsyncT in 1:length(tVector)) {
				...
				for (rsyncD in 1:length(dVector)) {
					...
					rsyncString<-paste("rsync -a bomeara@login.newton.utk.edu:/data/abc/RunsJan2012/ActualRuns/T",rsyncT,"/T",rsyncT,"_D",rsyncD,"/ /Users/bomeara/Sites/RunsJan2012/ActualRuns/T",rsyncT,"/T",rsyncT,"_D",rsyncD,"/",sep="")
					print(rsyncString)
					system(rsyncString)
				}
			}
