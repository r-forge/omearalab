...
GetPValue<-function(simdata, observed) {
	opt.quantile<-function(x, simdata, desired) {
		return(abs(desired - quantile(simdata, probs=x)))
	}
	percentage<-optimize(f=opt.quantile, interval=c(0,1), simdata=simdata, desired=observed)$minimum
	
	return(min(2*(1-percentage),2*percentage))
...
#  file.list<-system("ls -1 | grep total137.RSave",intern=TRUE)
  ...
  par(mfcol=c(1,1))
  plot(x=c(0,9), y=c(-.1,1), xlab="",ylab="Proportion", type="n",xaxt="n", bty="n")
  lines(x=c(-1,9), y=c(0,0))
  if (length(file.list)>1) {
    #pdf(file=paste(dirs[dir.index],".pdf",sep=""),height=4)
    #par(mfcol=c(2,4))
    proportions.mean<-rep(NA,8)
    valuesUniform<-quantile(rmultinom(100000,464,rep(1,8))[1,]/464,probs=c(0.025,0.5,0.975))
    ...
    for (i in sequence(8)) {
      ...
      small.dataset<-small.dataset[which(small.dataset[,3+1]==chars[3]),]
      small.dataset<-small.dataset[which(small.dataset[,4+1]==chars[4]),]
      observed<-dim(small.dataset)[1]/dim(original.data)[1]
	  lines(x=c(i-.3, i+.3), y=rep(observed,2),col="black",lty="dotted",lwd=2)
	  text(x=i,y=0, combo.names[i], pos=1)

      colors<-c("red", "blue", "darkgray")
      
      values137<-quantile(current[137,-1],probs=c(0.025,0.5,0.975))
      values151<-quantile(current[dim(current)[1],-1],probs=c(0.025,0.5,0.975))

      lines(x=rep(i-.2,2),y=values137[-2],lwd=2,col=colors[1])
      points(x=i-.2, y=values137[2], pch=20,col=colors[1])
      text(x=i-.2, y=0.03+values137[3], sprintf("%.2f", round(GetPValue(current[137,-1],observed),2)),pos=3, cex=0.7, srt=90,col=colors[1])
      
      lines(x=rep(i,2),y=values151[-2],lwd=2,col=colors[2])
      points(x=i, y=values151[2], pch=20,col=colors[2])
      text(x=i, y=0.03+values151[3], sprintf("%.2f", round(GetPValue(current[dim(current)[1],-1],observed),2)),pos=3, cex=0.7, srt=90,col=colors[2])

      lines(x=rep(i+.2,2),y=valuesUniform[-2], lwd=2,col=colors[3])
      points(x=i+.2, y=valuesUniform[2], pch=20, col=colors[3])
      text(x=i+.2, y=0.03+valuesUniform[3], sprintf("%.2f", round(GetPValue(rmultinom(100000,464,rep(1,8))[1,]/464,observed),2)),pos=3, cex=0.7, srt=90, col=colors[3])


      legend("topright", legend=c("present", "15 MY in future", "equal"), fill=colors, col=colors, bty="n" )
# # 
