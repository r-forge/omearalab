	...
	for (transitionIndex in 1:numberOfTransitionModelsForPartitionSize[partitionSize]) { 
		for (diversificationIndex in 1:numberOfDiversificationModelsForPartitionSize[partitionSize]) {
			if (min(numberOfTransitionModelsForPartitionSize[partitionSize],numberOfDiversificationModelsForPartitionSize[partitionSize])>0) { #just to make sure we have models: for (1:1) and for (1:0) still run
				...
				lsString=paste(paste("ls -1 ",paste("/Users/bomeara/Sites/Floral/RunsFebruary2011/ActualRuns/P",partitionSchemeText,sep="",collapse=""),"/",nameRoot,' | grep -c final.matrix.all',sep="",collapse=""))
				#print(lsString)
				...
				if(finalMatrixAllCount==1) {
					...
					lsString=paste(paste("ls -1 ",paste("/Users/bomeara/Sites/Floral/RunsFebruary2011/ActualRuns/P",partitionSchemeText,sep="",collapse=""),"/",nameRoot,' | grep final.matrix.all',sep="",collapse=""))
					...
					finalMatrixFullPath=paste(paste("/Users/bomeara/Sites/Floral/RunsFebruary2011/ActualRuns/P",partitionSchemeText,sep="",collapse=""),"/",nameRoot,'/',finalMatrixName,sep="")
